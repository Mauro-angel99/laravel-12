...
const showDeletePositionModal = ref(false)
...
const deletePosition = async () => {
  showDeletePositionModal.value = false
  if (!selectedPosition.value) return
  try {
    const res = await axios.delete(`/api/warehouse/positions/${selectedPosition.value.id}`)
    closeProductsModal()
    await fetchPositions(currentPage.value)
    showMessageModal('success', 'Successo', res.data.message || 'Posizione eliminata con successo')
  } catch (error) {
    const msg = error.response?.data?.message || 'Errore durante l\'eliminazione della posizione'
    showMessageModal('error', 'Errore', msg)
  }
}
...
<template>
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
          ...
          <div class="px-6 py-5 space-y-5">
              ...
              <table class="w-full text-sm">
                <thead class="bg-copam-blue text-white">
                  <tr>
                    ...
                    <th class="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider border-r border-blue-400/40">Ord. Prod.</th>
                    <th class="px-4 py-2.5 text-center text-xs font-semibold uppercase tracking-wider">Iniziato</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                  <tr v-if="selectedProducts.length === 0">
                    <td colspan="3" class="px-4 py-8 text-center text-xs text-gray-400">Nessuna merce in questa posizione</td>
                  </tr>
                  <tr
                  ...
                  >
                    ...
                    <td class="px-4 py-2.5 whitespace-nowrap text-gray-600 border-r border-gray-100">
                    ...
                    <td class="px-4 py-2.5 whitespace-nowrap text-center">
                      <span v-if="product.started" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700">Sì</span>
            ...
            <div class="flex justify-between border-t border-gray-100 pt-4">
              <button @click="showDeletePositionModal = true"
      ...
      <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <form @submit.prevent="updateWarehouse" class="px-6 py-5 space-y-4">
            <div class="flex items-center gap-3">
              <label class="flex items-center gap-2 cursor-pointer select-none">
                <input v-model="formData.started" type="checkbox" class="sr-only peer"/>
                <div class="relative w-10 h-5 bg-gray-200 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-copam-blue"></div>
  ...
  <!-- Modal Conferma Eliminazione Posizione -->
  <Teleport to="body">
    <div v-if="showDeletePositionModal" class="fixed inset-0 z-[75] overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 class="text-lg leading-6 font-medium text-gray-900">Svuota posizione</h3>
              <div class="mt-2">
                <p class="text-sm text-gray-500">
                  Sei sicuro di voler svuotare la posizione
                  <span class="font-semibold text-gray-800">{{ selectedPosition?.warehouse_position }}</span>?
                </p>
              </div>
            </div>
          </div>
          <div class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-2">
            <button type="button" @click="deletePosition"
            ...
            <button type="button" @click="showDeletePositionModal = false"
