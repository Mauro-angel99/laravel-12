<x-app-layout>
    ...
    <div class="py-1">
        <div class="max-w-8xl mx-auto sm:px-0 lg:px-0 xl:px-8">
            <div class="p-3 sm:p-3 bg-white shadow sm:rounded-lg">
                ...
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        <select name="per_page" id="per_page" onchange="this.form.submit()"
                            class="mt-1 block w-32 rounded-md border-gray-300 shadow-sm focus:border-copam-blue focus:ring-copam-blue sm:text-sm">
                ...
                @if (request('search'))
                    <div class="mb-4 text-sm text-gray-600">
                        @if ($users->count() > 0)
                            Trovati {{ $users->count() }} utente{{ $users->count() === 1 ? '' : 'i' }} per
                            "{{ request('search') }}"
                        @else
                                ...
                                @foreach ($users as $user)
                                    <tr class="hover:bg-gray-50 border-b border-gray-200">
                                        <td
                                            ...
                                            {{ $user->name }}</td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 border-r border-gray-200">
                                            {{ $user->email }}</td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 border-r border-gray-200">
